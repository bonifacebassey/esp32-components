...
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    {
        ESP_LOGE(TAG, "ESP_GAP_BLE_SCAN_RESULT_EVT... ");
        esp_ble_gap_cb_param_t *scan_result = (esp_ble_gap_cb_param_t *)param;
        ESP_LOGE(TAG, "scan_result: %d", scan_result->scan_rst.search_evt);
        switch (scan_result->scan_rst.search_evt)
        {
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            /* Search for BLE iBeacon Packet */
            ESP_LOGE(TAG, "Searching for BLE iBeacon Packet... ");
...
static esp_err_t ble_ibeacon_init()
{
...
#if (IBEACON_MODE == IBEACON_RECEIVER)
esp_err_t ble_ibeacon_start_receiver()
{
    esp_err_t status = ble_ibeacon_init();
    if (status == ESP_OK)
    {
...
#elif (IBEACON_MODE == IBEACON_SENDER)
esp_err_t ble_ibeacon_start_sender()
{
    esp_err_t status = ble_ibeacon_init();
    if (status == ESP_OK)
    {
